...
describe('Users', () => {
  ...
  describe('GET /api/users/me', () => {
    it('returns own profile without passwordHash', async () => {
      const res = await api.get('/api/users/me').set('Authorization', `Bearer ${adminToken}`);
    ...
    it('returns 401 with a malformed token', async () => {
      const res = await api.get('/api/users/me').set('Authorization', 'Bearer this.is.fake');
      expect(res.status).toBe(401);
    });
  });
  ...
  describe('GET /api/users', () => {
    it('returns 403 for viewer role', async () => {
      const res = await api.get('/api/users').set('Authorization', `Bearer ${viewerToken}`);
  ...
  describe('POST /api/users', () => {
    it('admin creates a user with specified role', async () => {
      const res = await api.post('/api/users').set('Authorization', `Bearer ${adminToken}`).send({
        name: 'New Analyst',
  ...
  describe('DELETE /api/users/:id', () => {
    ...
    it('rejects JWT for a user after that user is deleted', async () => {
      const email = `${randomUUID()}@test.com`;

      const created = await api
        .post('/api/users')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ name: 'Temp 2', email, password: 'password123' });
      const userId = created.body.data.user.id;

      const userToken = await getToken(email, 'password123');
      ...
      const me = await api.get('/api/users/me').set('Authorization', `Bearer ${userToken}`);
